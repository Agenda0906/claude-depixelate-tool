...
def _font_getsize(font: FreeTypeFont, text: str) -> Tuple[int, int]:
    """Compatibility shim: Pillow 10+ removed ImageFont.getsize."""
...
class OriginalImage:
    ...
    def text_size(self) -> Tuple[int, int]:
        return _font_getsize(self.image_creation_options.font, self.text)
...
def generate_image_from_text(text: str, options: ImageCreationOptions) -> OriginalImage:
    width, height = _font_getsize(options.font, text)
...
def generate_character_bounding_boxes(text: str, options: ImageCreationOptions) -> List[CharacterBoundingBox]:
    ...
    for i, char in enumerate(text):
        bottom_1 = _font_getsize(options.font, text[i])[1]
        right, bottom_2 = _font_getsize(options.font, text[:i + 1])
